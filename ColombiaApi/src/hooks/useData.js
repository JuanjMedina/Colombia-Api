...
import { getAllDepartments } from '../services/Depatament'

export const useData = () => {
  const { setData, setLoading, setError, setSelectOption } =
    useContext(dataConext)

  const searchData = ({ selectOption }) => {
    console.log('eleccion:', selectOption)
    if (selectOption === 'Presidentes') {
      ...
      setSelectOption(selectOption)
      return searchCitys()
    } else if (selectOption === 'Departamentos') {
      setSelectOption(selectOption)
      searchDepartament()
    } else {
